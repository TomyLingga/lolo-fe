...
export default function TaxesPage() {
  ...
  const [form, setForm] = useState({ name: "", percentage: "", type: "ADD" });
  ...
  function openForm(item?: Tax) {
    ...
    setForm(item ? { name: item.name, percentage: String(item.percentage), type: item.type } : { name: "", percentage: "", type: "ADD" });
    setFormOpen(true);
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      const payload = { name: form.name, percentage: Number(form.percentage), type: form.type as "ADD" | "DEDUCT" };
      ...
      <div className="p-4 sm:p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-800/60">
                <tr>{["Nama", "Persentase", "Tipe", "Status", "Aksi"].map(h => <th key={h} className="px-4 py-3 text-left table-header">{h}</th>)}</tr>
              </thead>
              <tbody>
                ...
                : filtered.map(item => (
                  <tr key={item.id} className={cn("table-row", !item.is_active && "opacity-40")}>
                    ...
                    <td className="px-4 py-3 text-slate-300">{item.percentage}%</td>
                    <td className="px-4 py-3">
                      <span className={cn("badge", item.type === "ADD" ? "badge-red" : "badge-green")}>
                        {item.type === "ADD" ? "Tambah (Pajak)" : "Kurang (Diskon)"}
                      </span>
                    </td>
        ...
        <Modal open={formOpen} onClose={() => setFormOpen(false)} title={editItem ? "Edit Tax/Discount" : "Tambah Tax/Discount"} size="sm">
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div><label className="label">Persentase (%) <span className="text-red-400">*</span></label>
              <input className="input" type="number" required min={0} max={100} step={0.01} value={form.percentage} onChange={e => setForm(p => ({ ...p, percentage: e.target.value }))} /></div>
            <div><label className="label">Tipe <span className="text-red-400">*</span></label>
              <select className="input" required value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value }))}>
                <option value="ADD">ADD — Tambah (Pajak)</option>
                <option value="DEDUCT">DEDUCT — Kurang (Diskon)</option>
              </select></div>
