...
export default function FreightForwardersPage() {
  ...
  async function handleDeactivate() {
    ...
    try { await freightForwardersApi.deactivate(deactivateItem.id); toast.success("Data dinonaktifkan"); setDeactivateItem(null); fetchData(); }
    catch (err) { toast.error(getErrorMessage(err)); }
    ...
    <AppLayout>
      <div className="p-4 sm:p-6">
        ...
        <ConfirmDialog open={!!deactivateItem} onClose={() => setDeactivateItem(null)} onConfirm={handleDeactivate}
          title="Nonaktifkan Freight Forwarder" message={`Nonaktifkan "${deactivateItem?.name}"?`} confirmLabel="Nonaktifkan" danger loading={deactivateLoading} />
      </div>
    </AppLayout>
  );
}
