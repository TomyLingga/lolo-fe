...
import { invoicesApi, freightForwardersApi } from "@/lib/api";
import { getUser } from "@/lib/auth";
import { formatDate, formatCurrency, getErrorMessage, cn } from "@/lib/utils";
import toast from "react-hot-toast";
import type { Invoice, FreightForwarder, Registration } from "@/types";
...
export default function InvoicesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await invoicesApi.getAll(params as { date_from?: string; date_to?: string });
      let d = res.data.data || [];
      if (tab !== "ALL") d = d.filter(i => i.status === tab);
      setData(d);
  ...
  useEffect(() => {
    ...
    freightForwardersApi.getAll().then(r => setFfs(r.data.data.filter(f => f.is_active))).catch(() => {});
    const today = new Date().toISOString().slice(0, 10);
    setCreateForm(p => ({ ...p, invoice_date: today }));
  }, [createOpen]);

  useEffect(() => {
    if (!selectedFf) { setInvoiceableRegs([]); return; }
    invoicesApi.getInvoiceableRegistrations(Number(selectedFf))
      .then(r => setInvoiceableRegs(r.data.data || [])).catch(() => {});
  }, [selectedFf]);

  const filtered = search
    ? data.filter(i => [i.invoice_number, i.freight_forwarder?.name].some(v => v?.toLowerCase().includes(search.toLowerCase())))
    : data;
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(true);
    try {
      await invoicesApi.create({ ...createForm, freight_forwarder_id: Number(selectedFf), registration_ids: selectedRegIds });
      toast.success("Invoice berhasil dibuat");
      setCreateOpen(false);
      setSelectedFf(""); setSelectedRegIds([]);
      fetchData();
  ...
  async function handleDeactivate() {
    ...
    try {
      ...
      setDeactivateConfirm(false); fetchData();
    } catch (err) { toast.error(getErrorMessage(err)); }
    finally { setActionLoading(false); }
  }
    ...
    <AppLayout>
      <div className="p-4 sm:p-6">
        ...
        <div className="card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={6} className="px-4 py-12 text-center text-slate-500">Memuat...</td></tr>
                ) : filtered.length === 0 ? (
                  ...
                  <tr key={inv.id} className={cn("table-row", !inv.is_active && "opacity-40")}>
                    ...
                    <td className="px-4 py-3 text-slate-300">{inv.total_amount ? formatCurrency(inv.total_amount) : "-"}</td>
                    <td className="px-4 py-3">
          ...
          <form onSubmit={handleCreate} className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select className="input" required value={selectedFf} onChange={e => { setSelectedFf(e.target.value); setSelectedRegIds([]); }}>
                ...
                {invoiceableRegs.length === 0 ? (
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto border border-slate-700 rounded-lg p-3">
                    {invoiceableRegs.map(r => (
                      <label key={r.id} className="flex items-center gap-3 cursor-pointer hover:bg-slate-800 p-2 rounded">
        ...
        <ConfirmDialog open={payConfirm} onClose={() => setPayConfirm(false)} onConfirm={handlePay}
          title="Tandai Invoice Lunas" message={`Tandai invoice ${selectedInv?.invoice_number} sebagai LUNAS?`}
        ...
        <ConfirmDialog open={deactivateConfirm} onClose={() => setDeactivateConfirm(false)} onConfirm={handleDeactivate}
          title="Nonaktifkan Invoice" message={`Nonaktifkan invoice ${selectedInv?.invoice_number}?`}
